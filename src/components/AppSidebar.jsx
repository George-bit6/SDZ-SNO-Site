...
import { LayoutDashboard, ClipboardList, Award, Users, CalendarDays, Settings, Shield, Trophy, CalendarRange, Boxes, X } from "lucide-react";
...
export const AppSidebar = ({ role, accentColor }) => {
    ...
    // Export the mobile menu toggle for use in Topbar
    if (typeof window !== 'undefined') {
        window.toggleMobileMenu = () => setIsOpen(prev => !prev);
    }
    ...
    return (
        <>
            ...
            {/* Sidebar */}
            <aside className={cn(
                "fixed inset-y-0 left-0 z-50 flex flex-col border-e border-gray-200 text-gray-800 transition-transform duration-300 md:translate-x-0 md:static md:z-auto md:bg-sidebar md:text-sidebar-foreground md:border-sidebar-border bg-white",
