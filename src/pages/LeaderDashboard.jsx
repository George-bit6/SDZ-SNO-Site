        ...
        <main className="flex-1 overflow-y-auto px-4 md:px-8 py-8">
          <div className="flex items-end justify-between mb-8 flex-wrap gap-4">
            <div>
              ...
              <h1 className="font-serif text-4xl md:text-5xl">{t("ld.title")}</h1>
            </div>
            ...
            <section className="rounded-lg border border-border bg-card shadow-card overflow-hidden">
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  ...
                  <tbody>
                    {members.map((m) => (<tr key={m.name} className="border-t border-border hover:bg-background/40 transition-colors">
            ...
            <div className="space-y-6">
              <section className="rounded-lg border border-border bg-card shadow-card p-6">
                ...
                <div className="space-y-2">
                  {members.slice(0, 6).map((m) => (<div key={m.name} className="flex items-center gap-2">
                      <span className="text-xs w-16 truncate text-muted-foreground">{m.initials}</span>
                      <div className="flex gap-1 flex-1">
                        {Array.from({ length: 7 }).map((_, i) => {
                const intensity = (m.progress + i * 7) % 100;
                return (<div key={i} className={`h-5 flex-1 rounded-sm ${heatColor(intensity)} opacity-${intensity > 50 ? "100" : "70"}`} title={`Week ${i + 1}: ${intensity}%`}/>);
            })}
                      </div>
                    </div>))}
                </div>
              ...
              <section className="rounded-lg border border-border bg-card shadow-card p-6">
                ...
                <ul className="space-y-3">
                  {reviews.map((r) => (<li key={r.name + r.task} className="rounded-md border border-border bg-background p-3">
